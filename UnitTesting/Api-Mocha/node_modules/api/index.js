...
module.exports = {
  ...
  validate_text: function (pText) {
    var isValid = false;
    var newText = pText.trim();
    var re = /<[a-z][\s\S]*>/i;
    newText = newText.replace(/\s+/g, '');
    try {
      isValid = validator.isAlphanumeric(newText,'es-ES');
      if (isValid === false){
        isValid = validator.isEmail(newText);
        if (isValid === false){
          isValid = !re.test(newText);

          console.log("Reulstado desp - "+isValid);
